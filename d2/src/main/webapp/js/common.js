$(document).ready(function() {	
    ...
    $("#modal-alert50").iziModal({
        title: "用户信息已存在，请换个手机号码进行注册。",
        iconClass: 'icon-check',
        headerColor: 'red',
        overlayClose: false,
        width: 400
		...
		$.ajax({  
			...
			success: function (res) {
	                ...
	                if(res.flag == 'true'){
	                	...
	                	$('.border-lump .phone').html( data_obj.phone);
			...
			$.ajax({  
				...
				success: function (res) {
					if(res.flag=='true'){
                        ...
                        $('#modal-alert50').iziModal('open');
                        $('#modal-alert50').on('click','.iziModal-button-close',function(){
                            //UnMaskIt($('.mask'));
                            window.location.reload();
                        });
					}
		...
		$.ajax({  
			...
			success: function (res) {  
					...
					if(res.flag == "true"){
						...
						var ErrorTxt = res.falseMsg;
                        $('#modal-alert2').iziModal('setTitle', ErrorTxt);
