...
$(document).ready(function(){
		...
		$.ajax({  
			...
			success: function (res) {  
	                ...
	                if(res.flag == "true"){
	                	...
	                	/* 编辑资料板块刷新数据 */
	                	$('.welcome-lump .phone').html(res.user.phone);
	                	$('.welcome-lump .point').html(res.user.point);
	                	// $('.show-message .id').html(res.user.id);
	                	$('.show-message .name').html(res.user.userName);
	                	$('.show-message .sex').html(res.user.sex);
	                	$('.show-message .year').html(res.user.age);
	                	$('.show-message .school').html(res.user.school);
	                	$('.show-message .major').html(res.user.major);
	                	$('.show-message .loc').html(res.user.loc);
	                	$('.show-message .rank').html(res.user.rank);
	                	$('.show-message .img-preview').attr("src",res.user.head_img)

	                	$('.change #name').val(res.user.userName);
	                	$('.change #sex').val(res.user.sex);
	                	$('.change #year').val(res.user.age);
	                	$('.change #school').val(res.user.school);
	                	$('.change #major').val(res.user.major);
	                	$('.change #loc').val(res.user.loc);
	                	$('.change #rank').val(res.user.rank);
                    ...
                    //
	                	...
	                	// /* Plus帖子板块刷新数据 */
	                	var plus_len = res.forun.length;
	                	if(plus_len > 0){
	                		$('.plus-fabu .message h4').removeClass("hide");
	                		for( var h = 0 ; h < plus_len ; h++){
	                			$('.plus-fabu .message ul').append('<li class="clearfix" id='+ res.forun[h].id +'><a href="#" class="no-wrap fl">'+ res.forun[h].tt +'</a><p class="de fl">删除该条发布信息</p></li>');
                                $('.plus-fabu .message ul li').eq(h).find("a").attr("href",res.forun[h].a_href);
	                		}
	                	...
	                	console.log(friends_len);
                        if(friends_len > 0){
                            for( var h = 0 ; h < friends_len ; h++){
                                /* 用户若无填写昵称、则显示“ 用户+ 注册手机号码 ” */
                                if( res.friend[h].userName == null){
                                    $('.friend-manage .friend-list').append('<li id='+ res.friend[h].id+'><img title="点击查看好友详情"><div class="mes"><p>用户'+ res.friend[h].phone+'</p></div><div class="del-friend">删除好友</div></li>');
                                }else{
                                    $('.friend-manage .friend-list').append('<li id='+ res.friend[h].id+'><img title="点击查看好友详情"><div class="mes"><p>'+ res.friend[h].userName+'</p></div><div class="del-friend">删除好友</div></li>');
                                }
                                $('.friend-list li').eq(h).find("img").attr('src', res.friend[h].head_img);
                            }
                            console.log("刷新好友列表....");
                        }else{
	...
	var upphone = $('.welcome-lump .phone').html();
	console.log(upphone);
/* 图片上传插件 */
var uploader = WebUploader.create({
    ...
    uploader.on('uploadBeforeSend',function( obj , data ,headers ){
		data.phone = $('.welcome-lump .phone').html()
...
$('.change-message').click(function(){
	var user_mes = {
		phone: $('.welcome-lump .phone').html(),
        userName: $('.edit-content #name').val(),
		sex: $('.edit-content #sex').val(),
		age: $('.edit-content #year').val(),
...
$('.change-pw').click(function(){
	var user_pw = {
		...
		password: $('.change-password .now').val()
	};
	/* 用户未填写的部分 直接用 保密 取代*/
...
//具体查看好友信息详情
$('.list-content').on('click','li img',function(){
	var f = {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if(res.phone != null){
                $('#modal-custom .name span').html(res.userName);
                $('#modal-custom .sex span').html(res.sex);
                $('#modal-custom .age span').html(res.age);
                $('#modal-custom .school span').html(res.school);
                $('#modal-custom .loc span').html(res.loc);
                $('#modal-custom .img-box img').attr("src",res.head_img);
